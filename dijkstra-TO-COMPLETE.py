...
class Graph():
    ...
    def dijsktra(self, start, end):                                
        ...
        ### COMPLETE CODE HERE ###
            ...
            self.Q.pop(self.getPopPosition(uNode)) # remove the node 
            
            for vNode in self.getUnvisitedNodes(uNode): # loop throught the unvisted node in Unode
                alt = self.dist[u] + self.weights[(uNode),(vNode)]  # assign the sum of the distance the wieght to alt
                v = self.getIndex(vNode) # call the function get index using vNode as the parmeter that goes in 
                if alt < self.dist[v]: # if alt is less than the distance of index v value 
                    self.dist[v] = alt # dist is assignes alt 
                    self.previous[v] = uNode # previous with index v is assigned uNode
        ...
        return shortest_path, self.dist[len(self.dist) -1]
